  ...
  return <>
      ...
      <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
        <DialogContent className="max-w-3xl p-0 border-bobsonm-navy bg-bobsonm-black/90 backdrop-blur-lg">
          <button onClick={() => setSelectedImage(null)} className="absolute right-4 top-4 z-10 rounded-full p-1.5 bg-black/50 text-white hover:bg-black/80 transition-colors" aria-label="Close">
            <X size={18} />
          </button>
          {selectedImage && <div className="flex items-center justify-center w-full">
              <img src={selectedImage.src} alt={selectedImage.alt} className="max-h-[80vh] w-auto object-contain" loading="lazy" />
            </div>}
        </DialogContent>
      </Dialog>
    </>;
